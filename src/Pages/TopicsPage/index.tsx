...
const TopicsPage: React.FC = () => {
  const [selectedTopics, setSelectedTopics] =
    useState<string[]>(getSelectedTopics());
  ...
  const checkSelectedTopics = () =>
    selectedTopics.filter((hate) => visibleTopics.includes(hate));
        ...
        <OptionsContainer direction={'row'} wrap={'wrap'}>
          {visibleTopics.map((topic) => (
            <OptionCardRound
              ...
              selected={isSelected(topic)}></OptionCardRound>
          ))}
        </OptionsContainer>
        <Button
