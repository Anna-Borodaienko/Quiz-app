...
const ProgressBarLine:React.FC<ProgressBarLineProps> = ({ currentPage }: ProgressBarLineProps) => {
  return (
    <BarContainer>
      <ProgressBar width="100%" color={colors.silver}>
        <ProgressBar width={`${currentPage*100/totalQuestions}%`} color={colors.magenta} />
      </ProgressBar>
    </BarContainer>
  )
}
